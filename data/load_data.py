...
def _load_mnist(path, dataset="training", digits=None, asbytes=False,
    ...
    try:
        part_of_path = r'C:\Users\seand\winter2022-hw6-neural-networks-sdchen6'
        
        
        #print(os.path.join(part_of_path, path, files[dataset][0]))
        #print(os.path.join(part_of_path, path, files[dataset][1]))
        #images_fname = 
        #labels_fname = 
        images_fname = os.path.join(part_of_path, path, files[dataset][0])
        labels_fname = os.path.join(part_of_path, path, files[dataset][1])
    except KeyError:
        raise ValueError("Data set must be 'testing' or 'training'")
